...
exports.getCookieHeaders = (
  ...
  const userData = JSON.stringify({
    UserAttributes: [
      ...
      {
        Name: 'cognito:groups',
        Value: decodedIdToken['cognito:groups'] || []
      }
    ],
  ...
  if (expireNoncePke) {
    cookies['spa-auth-edge-nonce'] = `none; ${cookieSettings['spa-auth-edge-nonce']}`;
    cookies['spa-auth-edge-pkce'] = `none; ${cookieSettings['spa-auth-edge-pkce']}`;
  }
...
exports.webserverRedirect = (request, config) => {
  if (config && config.ENABLED) {
    console.log('WebServer enabled');
    let { uri } = request;
    const excludePaths = config.EXCLUDE_PATHS || [];
    for (let i = 0; i < excludePaths.length; i += 1) {
      if (uri.startsWith(excludePaths[i])) {
        console.log(`Path to exclude ${excludePaths[i]} with uri ${uri}`);
        return request;
      }
    }
    if (uri.includes('.')) { // is file .csv .jpg etc
      console.log(`Path ${uri} is file`);
      return request;
    }
    if (config.SPA_MODE) {
      console.log(`SPA mode on in ${uri}. Returning root index.html`);
      return { ...request, ...{ uri: '/index.html' } };
    }
    if (uri.endsWith('/')) uri = uri.substring(0, uri.length - 1);
    console.log(`Path ${uri}/index.html`);
    return { ...request, ...{ uri: `${uri}/index.html` } };
  }
  return request;
};
